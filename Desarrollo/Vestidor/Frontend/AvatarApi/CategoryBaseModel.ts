...
export class CategoryBaseModel implements IAvatarEditorCategoryModel
{
    ...
    public selectColor(category: string, colorIndex: number, paletteId: number): void
    {
        ...
        categoryData.selectColorIndex(colorIndex, paletteId, category !== 'hd');
        
        const colorItem = categoryData.getSelectedColor(paletteId);

        if(colorItem.isDisabled)
        {
            categoryData.selectColorIndex(paletteIndex, paletteId, category !== 'hd');
    ...
    ): void
    {
        ...
        while(currentIds.length <= paletteId)
        {
            currentIds.push(
                categoryData.getSelectedColorId(
                    currentIds.length
                )
            );
        }

        currentIds[paletteId] = colorId;
    ...
    ): void
    {
        ...
        const colorItem =
            categoryData.selectCustomColorId(
                colorId,
                paletteId,
                category !== 'hd'
            );
        ...
        AvatarEditorUtilities.CURRENT_FIGURE
            .savePartSetColourId(
                category,
                categoryData.getSelectedColorIds(),
                true
            );
    }
